...
from ..schema import items, EmptyUpdate, ratings, orders, order_item, listings, sellers, users

#---------------------- Items----------------------#
def addItem(conn, name, description, category_id, lister_id):
    values = {"name":name,
...
def editItem(conn, item_id, lister_id, **kwargs):
...
def deleteItem(conn, item_id, lister_id):
    statement = items.update(
                    ).where(
                        and_(
...
def fetchItem(conn, item_id:int, user_id:int=0):
    can_review = 0
